...
class yc_AdminPageFramework_Form_View___Script_CollapsibleSection extends yc_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializeyc_AdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).on( 'yc_tech_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializeyc_AdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializeyc_AdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.yc_tech-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.yc_tech-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.yc_tech-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.yc_tech-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.yc_tech-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.yc_tech-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.yc_tech-collapsible-sections-title, .yc_tech-collapsible-section-title' )
            .enableyc_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.yc_tech-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'yc_tech-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.yc_tech-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.yc_tech-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.yc_tech-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.on( 'click', function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.yc_tech-sectionset' ).siblings().andSelf().find( '> .yc_tech-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.yc_tech-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .yc_tech-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.yc_tech-sections' ).children( '.yc_tech-section' ).children( '.yc_tech-section-table' ).children( 'caption' ).children( '.yc_tech-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enableyc_AdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( jQuery( nodeTarget ).hasClass( 'yc_tech-collapsible-button' ) ) {
                return false;
            }
        ...
        jQuery( this ).on( 'click', function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'yc_tech-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.yc_tech-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .yc_tech-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.yc_tech-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='yc_tech-collapsible-toggle-all-button-container'>" . "<span class='yc_tech-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
    }
